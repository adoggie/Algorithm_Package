...
/*        Determine ith pivot column and swap if necessary. */
	...
	int ttt =idamax_(&i__2, &vn1[i__], &c__1);//zfm 可删 2003-1-8
	pvt = i__ - 1 + idamax_(&i__2, &vn1[i__], &c__1);



  
//pvt的值决定了交换顺序 //zfm 2003-2-8
	if (pvt != i__) {
...
/*        Generate elementary reflector H(i). */
	...
	if (offpi < *m) {
	    i__2 = *m - offpi + 1;
	    dlarfg_(&i__2, &a_ref(offpi, i__), &a_ref(offpi + 1, i__), &c__1, 
		    &tau[i__]);
	} else {
	    dlarfg_(&c__1, &a_ref(*m, i__), &a_ref(*m, i__), &c__1, &tau[i__])
		    ;
	}
...
/*           Apply H(i)' to A(offset+i:m,i+1:n) from the left. */
	    ...
	    i__2 = *m - offpi + 1;
	    i__3 = *n - i__;
	    dlarf_("Left", &i__2, &i__3, &a_ref(offpi, i__), &c__1, &tau[i__],
...
/*        Update partial column norms. */
	...
	for (j = i__ + 1; j <= i__2; ++j) {
	    if (vn1[j] != 0.) {
/* Computing 2nd power */
		d__2 = (d__1 = a_ref(offpi, j), abs(d__1)) / vn1[j];
		temp = 1. - d__2 * d__2;
		temp = max(temp,0.);
/* Computing 2nd power */
		d__1 = vn1[j] / vn2[j];
		...
		if (temp2 == 1.) {
		    if (offpi < *m) {
			i__3 = *m - offpi;
			vn1[j] = dnrm2_(&i__3, &a_ref(offpi + 1, j), &c__1);
			vn2[j] = vn1[j];
		    } else {
			vn1[j] = 0.;
			vn2[j] = 0.;
