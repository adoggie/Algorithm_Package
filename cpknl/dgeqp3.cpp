...
///* Subroutine */extern "C" int dgeqp3_(long *m, long *n, double *a, long* 
//	lda, long *jpvt, double *tau, double *work, long *lwork,
//	 long *info)
...
/*  -- LAPACK routine (version 3.0) --   
    ...
    static integer jb, na, nb, sm, sn, nx;
...
/*     Move initial columns up front. */
	...
	if (jpvt[j] != 0) {
	    if (j != nfxd) {
		dswap_(m, &a_ref(1, j), &c__1, &a_ref(1, nfxd), &c__1);
		jpvt[j] = jpvt[nfxd];
		jpvt[nfxd] = j;
	    } else {
		jpvt[j] = j;
...
/* CC         CALL DORM2R( 'Left', 'Transpose', M, N-NA, NA, A, LDA,   
   CC  $                   TAU, A( 1, NA+1 ), LDA, WORK, INFO ) */
	    ...
	    dormqr_("Left", "Transpose", m, &i__1, &na, &a[a_offset], lda, &
		    tau[1], &a_ref(1, na + 1), lda, &work[1], lwork, info);
/* Computing MAX */
	    i__1 = iws, i__2 = (integer) work[1];
	    iws = max(i__1,i__2);
	}
...
/*        Initialize partial column norms. The first N elements of work   
	...
	for (j = nfxd + 1; j <= i__1; ++j) {
	    work[j] = dnrm2_(&sm, &a_ref(nfxd + 1, j), &c__1);
	    work[*n + j] = work[j];
/* L20: */
	}
...
/*
	...
	FILE* m_fp;
	m_fp = fopen("C:\\matrix.txt","a");
	
	fprintf(m_fp,"\n\nPrintMatrix\n\n");
    for(int idx = 1;idx<= 4 * 32; idx++)
	{
		fprintf(m_fp, "%f\t",work[idx]);
		if ((idx%4)==0)
			fprintf(m_fp,"\n");
	}
	fprintf(m_fp, "\n\n");
...
/*              Factorize JB columns among columns J:N. */
		...
		i__2 = j - 1;
		i__3 = *n - j + 1;
		dlaqps_(m, &i__1, &i__2, &jb, &fjb, &a_ref(1, j), lda, &jpvt[
			j], &tau[j], &work[j], &work[*n + j], &work[(*n << 1) 
			+ 1], &work[(*n << 1) + jb + 1], &i__3);
...
/*        Use unblocked code to factor the last or only block. */
	...
	if (j <= minmn) {
	    i__1 = *n - j + 1;
	    i__2 = j - 1;
	    dlaqp2_(m, &i__1, &i__2, &a_ref(1, j), lda, &jpvt[j], &tau[j], &
